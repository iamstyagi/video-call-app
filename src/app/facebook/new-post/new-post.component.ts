...
AssignedBy: assigendItem[];
 ...
 problemReportedValue:any
  ...
  getACname() {
    ...
    this.fbService.getFBDetails(data).subscribe((res: any) => {
      this.accountName = res[0].socialMediaUserScreenName
    })
  }
    ...
    this.fbService.getFBDetails(campId).subscribe((res: any) => {
      ...
      let data = {
        ...
        "selectedFacebookInboxRecord": {
          "fbMediaLink": this.successfulFileUploadResponse,
          "socialMediaUserScreenName": res[0].socialMediaUserScreenName,
          "text": this.fbPostTextArea,
          "accountID": res[0].accountID,
          "loginSocialMediaUser": res[0].loginSocialMediaUser,
          "inboxRowID": res[0].inboxRowID,
          "msgThreadID": res[0].msgThreadID,
          "socialMediaInReplyToUserID": res[0].socialMediaInReplyToUserID,
          "socialMediaMsgID": res[0].socialMediaMsgID,
          "mediaType": ""
        }
      }
...
alert(2)
    this.fbService.onFileUpload(file.files, this.campid).subscribe((res: any) => {
      this.successfulFileUploadResponse = res.MediaLink;
      if (res.status == 'success') {
        ...
        (data) => {
         if(data){
            ...
            if (phonedata && phonedata.status == 'success') {
                ...
                // this.customService.disposeCallApiwithoutTifcket(payLoad);
