...
export class MailSentComponent implements OnInit {
    ...
    getData() {
        let sentData = {
            ...
            selectedFolder: 'email-sent',
            count: 10,
            // isfolderClick:this.isfolderClick
        }
        sessionStorage.setItem('selectedFolderValue','email-sent')
        

        this.mailService.getEmailData(JSON.stringify(sentData)).subscribe((res: any) => {
            
            this.sentMails = res.listemailSentRecords;
